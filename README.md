...
### Renamed to Maccha Bazar

The site name is pulled from **Settings & Banner → Business name** almost everywhere (navbar, page
titles, footer). If your database still has the old business name from an earlier setup, open
`/admin/settings`, update **Business name** to `Maccha Bazar` (or whatever you'd like), and save — the
whole site updates immediately, no redeploy needed.

### Mobile app feel

- A bottom tab bar (Home, Shop, Care, Contact, WhatsApp) now shows on phones, like a native app, instead
  of the old floating WhatsApp button.
- The "Install app" prompt only appears on the homepage now, not on every page.
- The PWA service worker's cache version was bumped, which forces every visitor's browser to drop old
  cached pages and pick up the latest version after this deploy — this also fixes product photos that
  previously failed to show up on the shop grid for visitors who had an old cached copy.
...
(default login: `admin` / `macchabazar2026` — change this in `.env.local` before going live).
